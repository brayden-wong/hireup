...
import { SendMessageForm } from "~/app/(main)/(messages)/_components/send-message-form";
...
import type { Thread as ThreadType } from "~/lib/types/thread";
import { INITIAL_MESSAGE_LIMIT } from "~/constants/initial-message-limit";
import {
  ReplyMessageProvider,
  useReplyStore,
} from "~/app/(main)/(messages)/messages/[id]/_components/thread";
...
export const ArchivedThread = ({ url, thread }: ThreadProps) => {
  ...
  const addMessages = useCallback(
    (messages: Message[], where: "start" | "end") => {
      if (where === "start") {
...
type MessagesProps = {
  ...
  addMessages: (messages: Message[], where: "start" | "end") => void;
};
...
const Messages = ({
  ...
  useEffect(() => {
    ...
    addMessages(pageData.page.messages, "end");
...
function useWebsocketListener(thread: ThreadType) {
